...
<!-- 상단 바로가기 링크시 부드럽게 상단으로 이동하는 외부 라이브러리 js 임포트(아래) -->
<script src="/resources/home/js/jquery.smooth-scroll.min.js"></script>
<!-- 화면을 초기화 시키는 reset 스타일 임포트(아래) : 크로스브라이징 처리하기 위해 -->
<!-- 크롬브라우저, IE, 엣지, 사파리, 파이어폭스 등 h1, p, ul, div 태그의 크기가 조금씩 다르기 때문에 작업 결과가 모든 브라우저(크로싱브라우저)에 똑같이 보이게 하기 위한 reset.css(아래) -->
<link rel="stylesheet" href="/resources/home/css/reset.css">
<!-- 여기서부터는 사용자 정의형 스타일 + 스크립트 추가(아래) -->
<!-- 모바일용 CSS 임포트 -->
...
/* 태블릿용 메인 페이지 스타일 지정(아래) 801px~1065px(pc.css에서 정의된부분까지, 정의하지 않으면 무한대) */
@media all and (min-width: 801px) {
	
}

/* PC용 메인 페이지 스타일 지정(아래) 1066px~(정의하지 않으면 무한대까지) */
...
// 메인페이지 전용 슬라이드 호출 부분(아래)
$(document).ready(function() {
	// 위에서 선언한 함수|변수 사용(아래)
	// 여기서 함수 호출(실핼)
	slideAuto = setTimeout('play_w("right")', 3000); // 3초 마다 play_w 함수 실행
	...
	$('.rollstop').click(function() {
		...
		$(this).hide(); // 현재 보이는 rollstop버튼을 숨김.
		$('.rollplay').css('display','inline-block');
		if(slideAuto) {clearTimeout(slideAuto);} // slideAuto변수가 없다면, play_w함수를 중지한다.
	});
	$('.rollplay a').click(function() {
		$(this).parent().hide();
		$('.rollstop').css('display','inline-block');
		play_w('right'); // 3초마다 슬라이드 이미지 액션이 발생한다. play_w함수 안에 setTimeout이 있기 때문.
	});
	$('.rollingbtn li.seq a').each(function(index) {
		$(this).click(function() {
			$('.rollplay').hide();
			$('.rollstop').css('display','inline-block');
			if(slideAuto) {clearTimeout(slideAuto);} // 슬라이드 중지
			play_w(index); // 슬라이드 재생. 단, 시작위치는 현재 클릭한 인덱스부터.
		});
	});
});
</script>
